...
Source-of-truth for ``SESSION_OPEN`` / ``SESSION_CLOSE`` /
``SESSION_LAST_BAR_OPEN`` and the ``in_session()`` predicate. Re-exported
by :mod:`bot.schedule` for back-compat — new code should import from
here so it doesn't pull in the bot package."""
