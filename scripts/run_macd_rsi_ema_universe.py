"""Run the MACD+RSI+EMA strategy across the full NIFTY 500 universe.

Mirrors ``scripts/run_vwap_universe.py`` — loads the watchlist via
``bot.watchlist.WATCHLIST``, batches the yfinance fetch, sweeps the
registered ``macd_rsi_ema`` strategy across every symbol, writes:

  - results/sweep_macd_rsi_ema_universe.csv   (per-symbol summary)
  - results/macd_rsi_ema_universe_trades.parquet  (every trade, for the
                                                   dashboard's hour-of-day
                                                   and day-of-week buckets)

Run::

    python scripts/run_macd_rsi_ema_universe.py             # fetch + sweep
    python scripts/run_macd_rsi_ema_universe.py --no-fetch  # cache-only
"""
...
log = logging.getLogger("macd_rsi_ema_universe")

STRATEGY_NAME = "macd_rsi_ema"
OUT_SUMMARY = Path("results/sweep_macd_rsi_ema_universe.csv")
OUT_TRADES = Path("results/macd_rsi_ema_universe_trades.parquet")
...
def main() -> int:
    p = argparse.ArgumentParser(prog="scripts/run_macd_rsi_ema_universe.py")
    ...
    print()
    print(bar)
    print(f"MACD+RSI+EMA — UNIVERSE SWEEP   ({len(summary)} symbols)")
    print(bar)
