"""Run the VWAP+RSI+Order-Flow strategy across the full NIFTY 500 universe.

Mirrors ``scripts/run_vwap_universe.py``. Writes:

  - results/sweep_vwap_rsi_orderflow_universe.csv
  - results/vwap_rsi_orderflow_universe_trades.parquet

Note: the ``order flow`` (OI / PCR) confluence is NOT applied here —
F&O data isn't merged into the OHLCV pipeline. See
``strategies/vwap_rsi_orderflow.py`` for the TODO."""
...
log = logging.getLogger("vwap_rsi_orderflow_universe")

STRATEGY_NAME = "vwap_rsi_orderflow"
OUT_SUMMARY = Path("results/sweep_vwap_rsi_orderflow_universe.csv")
OUT_TRADES = Path("results/vwap_rsi_orderflow_universe_trades.parquet")
...
def main() -> int:
    p = argparse.ArgumentParser(
        prog="scripts/run_vwap_rsi_orderflow_universe.py",
    )
    ...
    print()
    print(bar)
    print(f"VWAP+RSI+ORDERFLOW — UNIVERSE SWEEP   ({len(summary)} symbols)")
    print(bar)
