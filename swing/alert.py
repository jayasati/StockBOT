"""Daily end-of-day swing alert dispatcher.
...
validated swing signal + regime filter, and sends qualifying tickers
to Telegram.

Strategy (validated by ``swing.replay`` — n=33-46, WR_3d ~37%,
...
CLI:
    python -m swing.alert                # send to Telegram (production)
    python -m swing.alert --dry-run      # print message, no Telegram
    python -m swing.alert --no-regime    # bypass regime gate (debug)
    python -m swing.alert --symbols 30   # quick run on 30 symbols (debug)
"""
...
from .data import fetch_alert_universe
from .evaluate import compute_breadth_series, evaluate_swing
...
log = logging.getLogger("swing.alert")
...
def evaluate_today(
    ...
    Thin wrapper over ``swing.evaluate.evaluate_swing(as_of_date=...)``."""
...
def main() -> None:
    ...
    stock_data = fetch_alert_universe(symbols)
